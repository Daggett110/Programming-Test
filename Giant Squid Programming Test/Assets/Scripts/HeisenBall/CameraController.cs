...
// to feel smooth, so better we get this working and work off of that
...
public class CameraController : MonoBehaviour {
    ...
    public float occludedCameraRotationSpeed = 1f;  // The speed at which our rotation should move back to normal during occlusion
    ...
    private void Start()
    {
        ...
        // Find the camera rigs current position relative to the rig
        rigOffset = transform.localPosition;

        // Note the original position/rotation of the camera relative to the parent so we can move back
        // to this placement when we need too
    ...
    void FixedUpdate ()
    {
        // If we are attempting to view the goals ..
        if (Input.GetButton("View Goal") && viewPoints.Length > 0)
        {
            ViewGoals();
        }
        else // .. otherwise, if we are not viewing the goals
        {
            CameraControl();
        }
    }
    ...
    {
        // reset the view point index after viewing goal. We need to reset this before viewing goals again
    ...
    private void ViewGoals()
    {
        ...
        // points like I wanted it too. I felt like I was on the right track, but I needed to budget time for other things.
        ...
        else // .. otherwise ..
        {
            ...
            // but it will give us a fairly smoothed move to it, slowing on approach 
            // and we can measure if we are within a certain distance, as shown 
            // in HasReachedCurrentViewPoint()
    ...
    {
        // If we just finished attempting to view the goal, we'll want to fix our orientation
        ...
        // Apply the right joystick's x axis to the object's y rotation in world space
        ...
        {
            // Apply the joystick's y axis to the object's x axis in local space
            transform.Rotate(Vector3.right * pitch * camSensitivity * Time.deltaTime);
            PitchRotation += pitch * camSensitivity * Time.deltaTime;
        }
        // It's important to keep these coordinate scopes in transform.Rotate() to ensure the 
        // camera is always vertical, and turns the appropriate direction
    }

    private void FixCameraObstructions()
    {
        // Establish a variable when we hit something, we can use to find the point at which we want to move our camera
        RaycastHit hit;

        // Target that we want our camera to be at. Typically this is the original camera position
        ...
        // so we can just have the player ignore itself
        ...
        if(Physics.Raycast(transform.position, -cam.forward, out hit, desiredCameraLocalPosition.magnitude))
        {
            ...
            // We'll cut this distance by 1/10th to safely avoid clipping with object's geometry as much as possible
            targetPosition *= 0.9f;

            // If for any reason, the camera is going to get too close to the player, bind it's nearest point
            // This means we just might clip with geometry if too, close, but it's better than clipping into our player
            if (targetPosition.magnitude < cameraNearBound)
            {
                // normalize it first to make scaling simpler
    ...
    private void ViewGoals()
    {
        // An old version of ViewGoals() is posted here, it's long and didn't smoothly move into and out of the view
